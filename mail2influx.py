...
def filter_postfix(line):
	...
	filter_by_tag = [
		['postfix_queued',
			"^.* postfix/qmgr\[.* from=.*, size=.*, nrcpt=.*$"],
		['postfix_relayed',
			"^.* postfix/.* to=.*, relay=.*, delay=.*, dsn=.*, status=sent .* queued as .*$"],
		['postfix_delivered',
			"^.* postfix/.* to=.*, relay=.*, delay=.*, dsn=.*, status=sent .*delivered via .* service.*$"],
		['postfix_sasl_auth',
			"^.* postfix/.* sasl_method=.* sasl_username=.*$"],
		['postfix_temp_fail',
			"^.* postfix/.* temporary failure.*$"],
		['postfix_connection',
			"^.* postfix.*: connect from .*$"],
		['postfix_tls_connect',
			"^.* postfix.*: Anonymous TLS connection established from .*$"],
		['postfix_nq_access_denied_client',
			"^.* postfix.*: NOQUEUE: reject: .* Access denied.*$"],
		['postfix_nq_mailbox_full',
			"^.* postfix.*: NOQUEUE: reject: .* Buzon de correo lleno / Mailbox is full.*$"],
		['postfix_nq_internal_error',
			"^.* postfix.*: NOQUEUE: reject: .* Internal error occurred.*$"],
		['postfix_nq_invalid_user_settings',
			"^.* postfix.*: NOQUEUE: reject: .* Invalid user settings.*$"],
		['postfix_nq_server_config_error',
		...
		['postfix_ssl_error',
			"^.* postfix.*: .* SSL_accept error from.*$"],
		['postfix_deferred_noroute',
			"^.* postfix.*: .* status=deferred .* No route to host.*$"],
		['postfix_deferred_service_not_available',
			"^.* postfix.*: .* status=deferred .*Recipient address rejected.* Service is unavailable .*$"],
		['postfix_deferred_unverified_recipient',
			"^.* postfix.*: .* status=deferred .*Recipient address rejected.* unverified address: .*$"],
		['postfix_deferred_over_quota',
			"^.* postfix.*: .* status=deferred .*would exceed mailbox quota.*$"],
		['postfix_deferred_over_quota',
			"^.* postfix.*: .* status=deferred .*Quota exceeded.*$"],
		['postfix_deferred_domain_not_found',
			"^.* postfix.*: .* status=deferred .*Host or domain name not found.*$"],
		['postfix_deferred_server_say_domain_not_found',
			"^.* postfix.*: .* status=deferred .*Recipient address rejected: Domain not found.*$"],
		['postfix_deferred_connection_timeout',
			"^.* postfix.*: .* status=deferred .*Connection timed out.*$"],
		['postfix_deferred_timeout_exceeded',
			"^.* postfix.*: .* status=deferred .*Error: timeout exceeded.*$"],
		['postfix_deferred_interrupted_while_talking',
			"^.* postfix.*: .* status=deferred .* with .* while .*$"],
		['postfix_deferred_connection_refused',
			"^.* postfix.*: .* status=deferred .*Connection refused.*$"],
		['postfix_deferred_greylisting',
			"^.* postfix.*: .* status=deferred .*Greylist.*$"],
		['postfix_deferred_too_many_recipients',
			"^.* postfix.*: .* status=deferred .*Too many recipients received from the sender.*$"],
		['postfix_deferred_server_error',
			"^.* postfix.*: .* status=deferred .*Client host rejected.* Server configuration error.*$"],
		['postfix_deferred_TLS_error',
			"^.* postfix.*: .* status=deferred .*Cannot start TLS.*$"],
		['postfix_deferred_no_route_to_host',
			"^.* postfix.*: .* status=deferred .*No route to host.*$"],
		['postfix_deferred_reject_hostname',
			"^.* postfix.*: .* status=deferred .*Client host rejected: cannot find your hostname.*$"],
		['postfix_deferred_temporary_error',
			"^.* postfix.*: .* status=deferred .* Temporary .*$"],
		['postfix_deferred_server_unreacheable',
